...
def perform_file_actions(plan: RenamePlan, args_ns: argparse.Namespace, cfg_helper, undo_manager: UndoManager) -> Dict[str, Any]:
    ...
    # --- Dry Run ---
    if args_ns.dry_run:
        log.info(f"--- DRY RUN for Batch ID: {batch_id} ---"); dry_run_messages = []
        ...
        for action in plan.actions:
            ...
            if target_exists_externally:
                ...
                # --- FIX: Catch specific exceptions here ---
    ...
    try:
        ...
        for action in plan.actions:
            # Ensure source exists unless it's a create_dir action? No, check later.
         ...
         # Handle conflicts detected in Phase 0 immediately
         log.error(f"Conflict check failed for batch {batch_id}: {e}")
         results['success'] = False; action_messages.append(f"ERROR: {e}")
         results['message'] = "\n".join(action_messages)
         # Re-raise ONLY FileExistsError if mode is fail
         if isinstance(e, FileExistsError) and conflict_mode == 'fail': raise e
         return results # Exit function gracefully for skip mode

    # --- If Phase 0 Passed, Proceed with Actions ---
    try:
        # --- Pre-action setup (Dir Creation, Backup) ---
        ...
        if plan.created_dir_path:
             ...
             if not target_dir.exists(): log.info(f"Creating folder: {target_dir}"); target_dir.mkdir(parents=True, exist_ok=True); created_dir = target_dir; undo_manager.log_action(batch_id=batch_id, original_path=target_dir, new_path=target_dir, item_type='dir', status='created_dir'); action_messages.append(f"CREATED DIR: '{target_dir}'")
             else: log.debug(f"Target directory already exists: {target_dir}")
        if action_type == 'backup': # Backup logic
            ...
            for action in plan.actions:
                ...
                backup_target = args_ns.backup_dir / orig_p.name; shutil.copy2(str(orig_p), str(backup_target)); log.debug(f"Backed up '{orig_p.name}'"); backed_up_count+=1
            if backed_up_count > 0: action_messages.append(f"Backed up {backed_up_count} files.")
            action_type = 'rename' # Proceed to rename phase

        # --- Perform Main Action ---
        if action_type == 'trash': # Trash Logic
            if not SEND2TRASH_AVAILABLE: raise FileOperationError("Send2Trash missing.")
            for action in plan.actions:
                orig_p=action.original_path; orig_p_resolved=orig_p.resolve(); final_p=resolved_target_map.get(orig_p_resolved)
                if not final_p: log.error(f"Trash skip: Missing path {orig_p}"); continue
                if not orig_p.exists(): log.warning(f"Cannot trash non-existent: {orig_p}"); continue
                undo_manager.log_action(batch_id=batch_id, original_path=orig_p, new_path=final_p, item_type='file', status='trashed')
                send2trash.send2trash(str(orig_p)); action_messages.append(f"TRASHED: '{orig_p.name}' (intended: '{final_p.name}')");
                results['actions_taken'] += 1 # Increment here
        elif action_type == 'stage': # Stage Logic
            if not args_ns.stage_dir: raise FileOperationError("Staging directory missing.")
            for action in plan.actions:
                orig_p=action.original_path; orig_p_resolved=orig_p.resolve(); final_p=resolved_target_map.get(orig_p_resolved)
                if not final_p: log.error(f"Stage skip: Missing path {orig_p}"); continue
                staged_path = args_ns.stage_dir / final_p.name
            ...
            log.debug(f"Starting Phase 1: Move to temporary paths for batch {batch_id}")
            for action in plan.actions:
                ...
                try:
                    ...
                    shutil.move(str(orig_p), str(temp_path)) # Use shutil.move consistently
                    # REMOVED exists check: if not temp_path.exists(): raise FileOperationError(...)
            ...
            # Phase 1 Rollback
            if not phase1_ok: #... rollback logic ...
                 log.warning(f"Rolling back Phase 1 for batch {batch_id}..."); # ... (using keywords) ...
                 results['success'] = False; results['message'] = "\n".join(action_messages); return results

            # Phase 2: Rename Temporary to Final
            log.debug(f"Starting Phase 2: Rename temporary paths to final for batch {batch_id}")
            ...
            for temp_path, final_path in temp_to_final_map.items():
                 ...
                 for orig_res, temp in original_to_temp_map.items():
                     # --- FIX: Compare resolved paths in lookup ---
                 ...
                 try:
                     ...
                     if original_path_for_log: #... log success update ...
    ...
    # --- Catch only unexpected Exceptions here ---
    except Exception as e:
        log.exception(f"Unhandled error during file actions for batch {batch_id}: {e}")
        ...
        if action_type == 'rename' and original_to_temp_map:
            log.critical("Unhandled error after temp files created. Rollback.")
            # (Rollback logic unchanged)
            for orig_p_res_rb, temp_p_rb in original_to_temp_map.items(): # ... rest of rollback ...
                 orig_p_rb = Path(orig_p_res_rb)
                 try:
                     if temp_p_rb.exists(): shutil.move(str(temp_p_rb), str(orig_p_rb)); undo_manager.update_action_status(batch_id=batch_id, original_path=str(orig_p_rb), new_status='failed_pending')
                     else: log.warning(f"Rollback skipped {orig_p_rb}, temp not found.")
                 except Exception as e_rb: log.error(f"Rollback Error for '{temp_p_rb.name}': {e_rb}"); action_messages.append(f"CRITICAL: Rollback failed for '{temp_p_rb.name}'.")
            if created_dir and created_dir.is_dir() and not any(created_dir.iterdir()):
                 try: created_dir.rmdir(); undo_manager.update_action_status(batch_id=batch_id, original_path=str(created_dir), new_status='reverted')
                 except OSError as e_rdir: log.error(f"Rollback could not remove dir '{created_dir}': {e_rdir}")
