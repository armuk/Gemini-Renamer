...
import time
import asyncio
import builtins # For print to stderr in critical cache error
import sys
...
def should_retry_api_error(exception: Exception) -> bool:
    ...
    if isinstance(exception, (ValueError, Exception)): # Check specific TVDB errors if possible
        msg = str(exception).lower()
        # For tvdb-v4-official, specific exceptions might be better if they exist (e.g., TVDBNotAuthorizedError)
        ...
        if '500' in msg or '502' in msg or '503' in msg or '504' in msg or 'timeout' in msg:
            ...
            return True
        if isinstance(exception, (AttributeError, TypeError, UnboundLocalError)): # These are likely programming errors
...
def find_best_match(title_to_find: str, api_results_tuple: Tuple[Dict, ...], result_key: str ='title', id_key: str ='id', score_cutoff: int = 70) -> Optional[Tuple[Dict, Optional[float]]]:
    ...
    if not THEFUZZ_AVAILABLE or not fuzz_process or not fuzz: # Check fuzz too
...
def _tmdb_results_to_dicts(results_iterable: Optional[Iterable[Any]], result_type: str = 'movie') -> Tuple[Dict[str, Any], ...]:
    ...
    except TypeError: # results_iterable might not be iterable
...
class MetadataFetcher:
    from rename_app.config_manager import ConfigHelper # For type hint
    ...
    def _sync_tmdb_movie_fetch(self, sync_title: str, sync_year_guess: Optional[int], sync_lang: str) -> Tuple[Optional[Any], Optional[Dict[str, Any]], Optional[float]]:
        ...
        try:
            ...
            if not results_list:
             ...
             status_code = getattr(e_search, 'status_code', 0)
    ...
    def _sync_tmdb_series_fetch(self, sync_title: str, sync_season: int, sync_episodes: Tuple[int, ...], sync_year_guess: Optional[int], sync_lang: str) -> Tuple[Optional[Any], Optional[Dict[int, Any]], Optional[Dict[str, Any]], Optional[float]]:
        ...
        if processed_results_list:
            ...
            if results_as_dicts_tuple:
                ...
                if self.tmdb_strategy == 'fuzzy' and THEFUZZ_AVAILABLE and fuzz:
                    ...
                    match_tuple = find_best_match(sync_title, results_as_dicts_tuple, result_key='name', id_key='id', score_cutoff=self.tmdb_fuzzy_cutoff)
                    if match_tuple: best_match_from_fuzzy_dict, temp_score = match_tuple
        ...
        try:
            show_details_obj = search.details(show_id_val, append_to_response="external_ids")
            if show_details_obj: final_show_data_obj_details = show_details_obj
        except TMDbException as e_details_tmdb:
             status_code_details = getattr(e_details_tmdb, 'status_code', 0)
        ...
        if sync_episodes and sync_season is not None:
            try:
    ...
    def _sync_tvdb_series_fetch(self, sync_title: str, sync_season_num: int, sync_episodes: Tuple[int, ...], sync_tvdb_id: Optional[int], sync_year_guess: Optional[int], sync_lang: str) -> Tuple[Optional[Dict], Optional[Dict[int, Any]], Optional[Dict[str, Any]], Optional[float]]:
        ...
        best_match_id_val: Optional[int] = sync_tvdb_id
        ...
        if show_data_dict and best_match_id_val is not None:
            try:
                ...
                page_num = 0
                while True:
                    episodes_page_data_dict = self.tvdb.get_series_episodes(best_match_id_val, page=page_num, lang=sync_lang) # type: ignore
                    if episodes_page_data_dict and isinstance(episodes_page_data_dict.get('episodes'), list):
                        ...
                        links_dict = self.tvdb.get_req_links() # type: ignore
                        if links_dict and links_dict.get('next'):
                             page_num += 1
                             log.debug(f"  Found 'next' link for TVDB episodes, fetching page {page_num}...")
                        else:
                            log.debug("  No 'next' link found for TVDB episodes or links structure unexpected. Assuming end of pages.")
                            break
                    else:
                        log.warning(f"TVDB episodes data invalid or missing 'episodes' key for page {page_num}, show ID {best_match_id_val}. Stopping pagination.")
                        break
                ...
                for ep_dict_item in all_episodes_list_tvdb:
                    if isinstance(ep_dict_item, dict):
                        api_season_num_val = ep_dict_item.get('seasonNumber')
                        api_ep_num_val = ep_dict_item.get('number')
                        if api_season_num_val is not None and api_ep_num_val is not None:
                            try:
                                if int(api_season_num_val) == int(sync_season_num):
                                    episodes_in_season_dict_tvdb[int(api_ep_num_val)] = ep_dict_item
                            except (ValueError, TypeError):
                                log.warning(f"Could not parse season/episode number from TVDB episode dict: {ep_dict_item}")
    ...
    async def _do_fetch_tmdb_movie(self, title_arg: str, year_arg: Optional[int], lang: str ='en') -> Tuple[Optional[Any], Optional[Dict[str, Any]], Optional[float]]:
        ...
        try:
            ...
            data_obj, ids_dict, score = await async_retryer(
                self._run_sync, self._sync_tmdb_movie_fetch, str(title_arg), year_arg, str(lang)
            )
    ...
    async def _do_fetch_tmdb_series(self, title_arg: str, season_arg: int, episodes_arg: Tuple[int, ...], year_guess_arg: Optional[int] = None, lang: str ='en') -> Tuple[Optional[Any], Optional[Dict[int, Any]], Optional[Dict[str, Any]], Optional[float]]:
        max_attempts = max(1, int(self.cfg('api_retry_attempts', 3)))
        wait_seconds = float(self.cfg('api_retry_wait_seconds', 2.0))
        async_retryer = AsyncRetrying(
            stop=stop_after_attempt(max_attempts),
        ...
        try:
            ...
            show_obj, ep_map, ids_dict, score = await async_retryer(
                self._run_sync, self._sync_tmdb_series_fetch, str(title_arg), int(season_arg), tuple(episodes_arg), year_guess_arg, str(lang)
            )
            if show_obj is None:
    ...
    async def _do_fetch_tvdb_series(self, title_arg: str, season_num_arg: int, episodes_arg: Tuple[int, ...], tvdb_id_arg: Optional[int] = None, year_guess_arg: Optional[int] = None, lang: str = 'en') -> Tuple[Optional[Dict], Optional[Dict[int, Any]], Optional[Dict[str, Any]], Optional[float]]:
        max_attempts = max(1, int(self.cfg('api_retry_attempts', 3)))
        wait_seconds = float(self.cfg('api_retry_wait_seconds', 2.0))
        async_retryer = AsyncRetrying(
            stop=stop_after_attempt(max_attempts),
        ...
        try:
            ...
            show_dict, ep_map, ids_dict, score = await async_retryer(
                self._run_sync, self._sync_tvdb_series_fetch, str(title_arg), int(season_num_arg), tuple(episodes_arg), tvdb_id_arg, year_guess_arg, str(lang)
            )
    ...
    async def fetch_series_metadata(self, show_title_guess: str, season_num: int, episode_num_list: Tuple[int, ...], year_guess: Optional[int] = None) -> MediaMetadata:
        ...
        while source_queue:
            ...
            if cached_data:
                try:
                    source_data, cached_ids_with_map, source_score = cached_data
                    if cached_ids_with_map and isinstance(cached_ids_with_map, dict) and '_ep_map' in cached_ids_with_map:
                        source_ep_map = cached_ids_with_map.pop('_ep_map', {})
                        source_ids = cached_ids_with_map
                    else:
                        source_ids = cached_ids_with_map if isinstance(cached_ids_with_map, dict) else {}
                        source_ep_map = {}
                ...
                try:
                    ...
                    if source == 'tmdb' and self.tmdb:
                        source_data, source_ep_map, source_ids, source_score = await self._do_fetch_tmdb_series(
                         ...
                         tmdb_ids_dict_for_tvdb = results_by_source.get('tmdb', {}).get('ids') or {}
                         tvdb_id_from_tmdb = tmdb_ids_dict_for_tvdb.get('tvdb_id') if isinstance(tmdb_ids_dict_for_tvdb, dict) else None
                    ...
                    log.error(f"Unexpected error during {source.upper()} series fetch for '{show_title_guess}' S{season_num}: {type(e_fetch_unexp).__name__}: {e_fetch_unexp}", exc_info=True)
            ...
            if source_has_show_data and source_has_all_episodes:
                ...
                log.info(f"{source.upper()} did not find show data. Error: {source_error or 'N/A'}. Will check next preferred source if available.")

        if not primary_source:
            for source_pref in source_preference:
                 if results_by_source[source_pref]['data'] is not None:
                     log.warning(f"Using {source_pref.upper()} as fallback primary source (found show data, but possibly missing episodes as preferred source was incomplete or failed).")
                     primary_source = source_pref
                     break
        
        if primary_source:
            primary_ids_dict = results_by_source[primary_source].get('ids') or {}
            merged_ids.update(primary_ids_dict)
            for source_other in source_preference:
                if source_other != primary_source:
                    other_ids_dict = results_by_source[source_other].get('ids') or {}
                    for k, v in other_ids_dict.items():
                         if v is not None and k not in merged_ids:
                             merged_ids[k] = v
        ...
        if primary_source:
            ...
            try:
                ...
                if episode_num_tuple and primary_ep_map_dict:
                    for ep_num_val in episode_num_tuple:
                        ...
                        if ep_details_obj:
                            ep_title_val: Optional[str] = None
                            air_date_val: Optional[str] = None
                            if isinstance(ep_details_obj, AsObj):
                                ...
                                ep_title_val = ep_details_obj.get('name') or ep_details_obj.get('episodeName')
                                air_date_val = ep_details_obj.get('air_date') or ep_details_obj.get('aired')
            ...
            for source_name in source_preference:
                if results_by_source[source_name]['error']:
                    error_messages.append(f"{source_name.upper()} Error: {results_by_source[source_name]['error']}")
            
            if error_messages:
                last_error_message_from_sources = " | ".join(error_messages)
            else:
                last_error_message_from_sources = "Metadata fetch failed from all sources (no specific errors, but no data found)."

        if not final_meta.source_api:
             log.warning(f"Metadata fetch/population ultimately failed for series: '{show_title_guess}' S{season_num}E{episode_num_tuple}. Reason: {last_error_message_from_sources or 'Unknown reason'}")
