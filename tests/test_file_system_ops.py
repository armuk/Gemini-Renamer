...
def mock_undo_manager(mocker):
    ...
    mock.update_action_status = MagicMock(return_value=True)
    return mock
...
def create_test_plan(
    ...
    # Source file existence is handled by individual tests now
    ...
    for orig_rel_name, new_rel_name, item_type, action_type in actions:
        ...
        # Parent dir for original path ensured by test setup now
...
def test_perform_file_actions_dry_run_move_create_dir(tmp_path, mock_cfg_helper, mock_undo_manager, mocker):
    ...
    # This should now work as mkdir is not mocked
...
def test_perform_file_actions_live_move_create_dir(tmp_path, mock_cfg_helper, mock_undo_manager, mocker):
    ...
    if created_dir_path.exists(): shutil.rmtree(created_dir_path)
    # Act
    result = file_system_ops.perform_file_actions(plan, mock_cfg_helper.args, mock_cfg_helper, mock_undo_manager)
    # Assert
    assert result['success'] is True, f"Action failed: {result.get('message')}"
    ...
    # Use .get() for safety in assertion checking kwargs
...
def test_perform_file_actions_live_trash(tmp_path, mock_cfg_helper, mock_undo_manager, mocker):
    ...
    result = file_system_ops.perform_file_actions(plan, mock_cfg_helper.args, mock_cfg_helper, mock_undo_manager)
    assert result['success'] is True; assert result['actions_taken'] == 1, f"Action count mismatch: {result['actions_taken']}"
...
def test_perform_file_actions_live_backup(tmp_path, mock_cfg_helper, mock_undo_manager, mocker):
    ...
    # Assert
    assert result['success'] is True, f"Action failed: {result.get('message')}"
    # --- FIX: Assert actions_taken based on successful log update ---
    # It performs ONE rename action after the backup
...
# --- Conflict Tests ---
# (test_perform_file_actions_conflict_skip unchanged)
...
# (test_perform_file_actions_conflict_fail unchanged)
...
# TODO: Add tests for transactional rollback scenarios